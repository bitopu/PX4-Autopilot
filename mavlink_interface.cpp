...
/**
 * @brief Handles incoming MAVLink messages and processes actuator control commands.
 *
 * This function processes MAVLink messages, specifically HIL_ACTUATOR_CONTROLS messages,
 * which contain actuator control values from the flight controller. The actuator controls
 * are stored in input_reference_ for use by the simulation.
 *
 * @param[in] msg Pointer to the MAVLink message to be processed
 * @param[out] received_actuator Flag set to true when actuator controls are successfully received
 *
 * @note input_reference_ is a vector that stores the normalized actuator control values 
 *       (typically ranging from -1.0 to 1.0) for each output channel. These values represent
 *       the commanded positions/speeds for actuators such as motor throttles, control surfaces,
 *       or other effectors in the simulation.
 *
 * @note This function is thread-safe and uses actuator_mutex to protect shared resources.
 * @note The armed state is extracted from the control mode flags.
 * @note Input indices are mapped 1:1 with the control array indices.
 */
/**
 * @brief Handles incoming MAVLink messages and updates interface state accordingly.
 *
 * This function processes specific MAVLink message types and updates the internal state
 * of the MavlinkInterface based on the message content.
 *
 * @param msg Pointer to the MAVLink message to be processed
 * @param received_actuator Reference to a boolean flag that is set to true when
 *                         actuator control messages are successfully received and processed
 *
 * Supported message types:
 * - MAVLINK_MSG_ID_HIL_ACTUATOR_CONTROLS: Processes hardware-in-the-loop actuator control
 *   commands. Updates armed state, input indices, and input reference values. Thread-safe
 *   with mutex protection.
 * - MAVLINK_MSG_ID_MANUAL_CONTROL: Processes manual control inputs and stores button states
 *   in manual_buttons_.
 *
 * @note The function uses mutex locking when handling actuator controls to ensure thread safety.
 * @note Input references are resized to n_out_max and populated with control values.
 */
void MavlinkInterface::handle_message(mavlink_message_t *msg, bool &received_actuator) {
  switch (msg->msgid) {
    ...
    case MAVLINK_MSG_ID_MANUAL_CONTROL: {
      ...
      manual_buttons_.store(manual_control.buttons);
      break;
    }
    default:
      break;
...
uint16_t MavlinkInterface::GetManualButtons() { return manual_buttons_.load(); }
